...
void setupRelays() {
  ...
  pinMode(relay3Pin, OUTPUT); // Setup new relay pin
  pinMode(relay4Pin, OUTPUT); // Setup relay4 pin
  pinMode(relay5Pin, OUTPUT);
  pinMode(relay6Pin, OUTPUT);
  digitalWrite(relay1Pin, HIGH);
  digitalWrite(relay2Pin, HIGH);
  digitalWrite(relay3Pin, HIGH); // Set new relay pin to HIGH initially
  digitalWrite(relay4Pin, HIGH); // Set relay4 pin to HIGH initially
...
void handleGrainDispensing(float currentWeight) {
  const float pricePerGram = 1.20; // Placeholder for the price per gram
  const String currency = "Rs.";   // Placeholder for the currency
  ...
  while (true) {
    ...
    if (key) {
      if (key >= '0' && key <= '9') {
        ...
        if (input.length() > 0) {
          targetWeight = input.toFloat() - 20; // Subtract 20 grams from the input
          break;
        } else {
          lcd.clear();
...
void runSealingProcess() {
  ...
  // Turn on the new relay at the beginning
